<script lang="ts">
	...
	import { ambilPengguna, ambilKelas, type PenggunaAPI, type KelasAPI } from '$lib/api/guru';
	...
	let kelas = $state<KelasAPI | null>(null);
	let namaKelas = $state('Belum ada kelas');
	...
	async function muatProfil() {
		...
		pesanError = '';
		try {
			const [sData, pData] = await Promise.all([ambilSiswa(s.siswaId), ambilPengguna(s.penggunaId)]);
			siswa = sData;
			pengguna = pData;
			if (sData.kelasId) {
...
<div class="space-y-4 px-4 pb-2">
	...
	{:else}
		<!-- ============ Kartu Profil ============ -->
		<div
			class="rounded-3xl border-b-4 border-b-[#12243f] bg-gradient-to-br from-primary to-primary/90 p-5 text-center text-white shadow-lg shadow-primary/20 mt-4"
		>
			<p class="text-[11px] font-black tracking-widest text-white/60 uppercase">Profil Siswa</p>
			<div
			...
			>
				{inisial(siswa?.namaLengkap ?? '')}
			</div>
			<h1 class="mt-4 text-lg font-black">{siswa?.namaLengkap ?? 'Siswa'}</h1>
			<p class="mb-4 text-xs text-white/70">Peserta Didik</p>
		...
		<!-- ============ Informasi Umum ============ -->
		<div
			class="rounded-2xl border-2 border-b-4 border-[#E2E8F0] border-b-[#CBD5E1] bg-white p-4 shadow-sm mt-4"
		>
			...
			<div class="divide-y divide-slate-100">
				<div class="flex items-center gap-3 py-3">
					...
					<div class="flex-1">
						<p class="text-[11px] font-bold tracking-wide text-slate-400 uppercase">NISN</p>
				...
				<div class="flex items-center gap-3 py-3">
					...
					<div class="flex-1">
						<p class="text-[11px] font-bold tracking-wide text-slate-400 uppercase">Kelas</p>
						<p class="text-sm font-bold text-slate-800">{namaKelas}</p>
					</div>
				</div>
				<div class="flex items-center gap-3 py-3">
					<div
					...
					<div class="flex-1">
						...
						<p class="text-sm font-bold text-slate-800">
							{akunAktif ? 'Siswa Aktif' : 'Nonaktif / Lulus'}
						</p>
					</div>
					<span
