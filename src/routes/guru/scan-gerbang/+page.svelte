<script lang="ts">
	...
	import { scanGateAbsensi } from '$lib/api/absensi';
	...
	function handleScanSuccess(decodedText: string) {
		...
		status = 'loading';
		message = 'Memproses scan kehadiran...';

		scanGateAbsensi({ kunciQr: decodedText })
			.then(() => {
				status = 'success';
				message = 'Berhasil! Kehadiran siswa tercatat.';
				
				// Play success sound if needed, for now just wait then standby
				setTimeout(() => {
					status = 'standby';
					message = '';
				}, 2000);
			...
			.catch((err) => {
				status = 'error';
				message = err instanceof Error ? err.message : 'QR Code tidak valid atau terjadi kesalahan.';
...
<div class="px-4 py-4 max-w-md mx-auto space-y-4">
	<div class="flex items-center gap-3">
		...
		<h1 class="text-xl font-black text-primary">Scan Gerbang</h1>
	</div>
	...
	<div class="rounded-3xl border-b-4 border-b-[#12243f] bg-gradient-to-br from-primary to-primary/90 p-5 text-white shadow-lg shadow-primary/20">
		<p class="text-[11px] font-black tracking-widest text-white/60 uppercase">Absensi Harian</p>
		<p class="text-sm font-bold mt-1">Arahkan kamera ke QR Code milik siswa untuk mencatat kedatangan mereka hari ini.</p>
	</div>

	<div class="rounded-2xl border-2 border-b-4 border-[#E2E8F0] border-b-[#CBD5E1] bg-white p-2 shadow-sm relative overflow-hidden">
		...
		{#if status === 'success'}
			<div class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-emerald-50/95 backdrop-blur-sm rounded-2xl p-6 text-center">
				...
				<h2 class="mt-4 text-lg font-black text-emerald-700">Scan Berhasil!</h2>
				<p class="mt-1 text-xs font-bold text-emerald-600/80">{message}</p>
			</div>
		{/if}
